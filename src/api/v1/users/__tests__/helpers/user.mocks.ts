/**
 * User Mocks - Jest Mock Factories for Unit Tests
 *
 * These factories create properly typed Jest mocks for user-related
 * interfaces. Use them to mock dependencies in unit tests.
 *
 * Key characteristics:
 * - Fully typed with jest.Mocked<T>
 * - All methods are jest.fn() by default
 * - Ready to configure with mockResolvedValue, mockRejectedValue, etc.
 *
 * @example
 * const mockRepo = createMockUserRepository();
 * mockRepo.findById.mockResolvedValue(userFixture);
 * const service = new UserServices(mockRepo, mockRoleService);
 */
...
export const userMockScenarios = {
  ...
  emailAlreadyTaken: (
    ...
    existingUser: ReturnType<typeof import("./user.fixtures").createUserFixture>
  ) => {
